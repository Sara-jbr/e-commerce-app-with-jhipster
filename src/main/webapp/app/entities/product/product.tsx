  ...
  return (
    <div>
      <h2 id="product-heading" data-cy="ProductHeading">
        <Translate contentKey="eCommerceAppJhipsterApp.product.home.title">Products</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="eCommerceAppJhipsterApp.product.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/product/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="eCommerceAppJhipsterApp.product.home.createLabel">Create new Product</Translate>
          </Link>
        </div>
      </h2>
        ...
        {productList && productList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="eCommerceAppJhipsterApp.product.id">ID</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                <th className="hand" onClick={sort('name')}>
                  <Translate contentKey="eCommerceAppJhipsterApp.product.name">Name</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('name')} />
                </th>
                <th className="hand" onClick={sort('description')}>
                  <Translate contentKey="eCommerceAppJhipsterApp.product.description">Description</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('description')} />
                </th>
                <th className="hand" onClick={sort('price')}>
                  <Translate contentKey="eCommerceAppJhipsterApp.product.price">Price</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('price')} />
                </th>
                <th className="hand" onClick={sort('productSize')}>
                  <Translate contentKey="eCommerceAppJhipsterApp.product.productSize">Product Size</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('productSize')} />
                </th>
                <th className="hand" onClick={sort('image')}>
                  <Translate contentKey="eCommerceAppJhipsterApp.product.image">Image</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('image')} />
                </th>
                <th>
                  <Translate contentKey="eCommerceAppJhipsterApp.product.productCategory">Product Category</Translate>{' '}
            ...
            <tbody>
              {productList.map((product, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{product.price}</td>
                  <td>
                    <Translate contentKey={`eCommerceAppJhipsterApp.Size.${product.productSize}`} />
                  </td>
                  <td>
                    {product.image ? (
                      <div>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="eCommerceAppJhipsterApp.product.home.notFound">No Products found</Translate>
            </div>
          )
        )}
      </div>
