  ...
  return (
    <div>
      <h2 id="shopping-cart-heading" data-cy="ShoppingCartHeading">
        <Translate contentKey="eCommerceAppJhipsterApp.shoppingCart.home.title">Shopping Carts</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="eCommerceAppJhipsterApp.shoppingCart.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/shopping-cart/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="eCommerceAppJhipsterApp.shoppingCart.home.createLabel">Create new Shopping Cart</Translate>
          </Link>
        </div>
      </h2>
        ...
        {shoppingCartList && shoppingCartList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="eCommerceAppJhipsterApp.shoppingCart.id">ID</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                <th className="hand" onClick={sort('placedDate')}>
                  <Translate contentKey="eCommerceAppJhipsterApp.shoppingCart.placedDate">Placed Date</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('placedDate')} />
                </th>
                <th className="hand" onClick={sort('status')}>
                  <Translate contentKey="eCommerceAppJhipsterApp.shoppingCart.status">Status</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('status')} />
                </th>
                <th className="hand" onClick={sort('totalPrice')}>
                  <Translate contentKey="eCommerceAppJhipsterApp.shoppingCart.totalPrice">Total Price</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('totalPrice')} />
                </th>
                <th className="hand" onClick={sort('paymentMethod')}>
                  <Translate contentKey="eCommerceAppJhipsterApp.shoppingCart.paymentMethod">Payment Method</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('paymentMethod')} />
                </th>
                <th className="hand" onClick={sort('paymentReference')}>
                  <Translate contentKey="eCommerceAppJhipsterApp.shoppingCart.paymentReference">Payment Reference</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('paymentReference')} />
                </th>
                <th>
                  <Translate contentKey="eCommerceAppJhipsterApp.shoppingCart.customerDetails">Customer Details</Translate>{' '}
            ...
            <tbody>
              {shoppingCartList.map((shoppingCart, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>
                    <Translate contentKey={`eCommerceAppJhipsterApp.OrderStatus.${shoppingCart.status}`} />
                  </td>
                  <td>{shoppingCart.totalPrice}</td>
                  <td>
                    <Translate contentKey={`eCommerceAppJhipsterApp.PaymentMethod.${shoppingCart.paymentMethod}`} />
                  </td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="eCommerceAppJhipsterApp.shoppingCart.home.notFound">No Shopping Carts found</Translate>
            </div>
          )
        )}
      </div>
